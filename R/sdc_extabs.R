...
sdc_extabs <- function(data, 
  ...
  original_data_with_statistics <- data
  ...
  if(is.null(ID)) {
    ...
    original_data_with_statistics$.ROW_NUMBER <- 1:nrow(data)
 
    message("ID variable not provided. ID .ROW_NUMBER created.")

  } # end if 

  # Provide message if no weight provided, set wgtthreshold to NULL ------------
  ...
  original_data_with_statistics <- 
    merge(original_data_with_statistics,
...
print.sdc_extabs <- function(x,
  ...
  if(!is.null(x$options$threshold)) {
    ...
    count_string <- paste("Unweighted count <", 
                          x$options$threshold)
